...
public class PlayerController : MonoBehaviour
{
    ...
    {
        //keyDashHorizontal = Input.GetAxisRaw("Horizontal");
        keyDashLeft = Input.GetKeyDown(KeyCode.LeftArrow);
        keyDashRight = Input.GetKeyDown(KeyCode.RightArrow);
        if (dashDirection == 0)
        {
            if (keyDashLeft)
            {
